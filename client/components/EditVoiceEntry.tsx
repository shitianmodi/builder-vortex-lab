...
export default function EditVoiceEntry({
  isOpen,
  onClose,
  onSave,
  initialData,
  ...
  const [startMinutes, setStartMinutes] = useState(
    initialData?.startTime.minutes || 0,
  );
  const [startSeconds, setStartSeconds] = useState(
    initialData?.startTime.seconds || 0,
  );
  const [endMinutes, setEndMinutes] = useState(
    initialData?.endTime.minutes || 0,
  );
  const [endSeconds, setEndSeconds] = useState(
    initialData?.endTime.seconds || 0,
  );
  const [transcription, setTranscription] = useState(
    initialData?.transcription ||
      "- 对话内容转文字 对话内容转文字 对话内容转文字 对话内容转文字 对话内容转文���\n- 对话内容转文字 对话内容转文字 对话内容转文字 对话内容转文字 对话内容转文字",
  );
  ...
  const adjustTime = (
    type: "startMinutes" | "startSeconds" | "endMinutes" | "endSeconds",
    direction: "up" | "down",
    amount: number = 1,
  ) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="w-[468px] h-[768px] bg-white rounded-3xl relative">
        ...
        <div className="flex justify-between items-center px-6 py-5 h-16">
          <h2 className="text-[#3D526C] text-lg font-semibold">
            编辑条目/语音
          </h2>
          <button
            onClick={onClose}
            ...
            <div className="flex-1 bg-[#F4F4F5] rounded-[32px] h-[52px] flex items-center px-4 gap-4">
              <button className="w-9 h-9 bg-[#004DA9] rounded-full flex items-center justify-center">
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path
                    d="M3.64 13.7V2.3C3.64 1.958 3.774 1.688 4.042 1.49C4.31 1.292 4.598 1.262 4.906 1.4L12.946 6.1C13.254 6.238 13.408 6.478 13.408 6.82V9.18C13.408 9.522 13.254 9.762 12.946 9.9L4.906 14.6C4.598 14.738 4.31 14.708 4.042 14.51C3.774 14.312 3.64 14.042 3.64 13.7Z"
                    fill="white"
                  />
                </svg>
              </button>
